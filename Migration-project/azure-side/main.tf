...
# Storage Account for Replication Cache 
...
# Network Security Group for the migrated VM
...
resource "azurerm_windows_virtual_machine" "replication" {
  ...
  source_image_reference {
    publisher = "MicrosoftWindowsServer"
    offer     = "WindowsServer"
    sku       = "2022-Datacenter-g2"  # Must be 2022 — installer fails on 2019
    version   = "latest"
  }
  tags = var.tags
}
